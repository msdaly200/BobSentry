...
Produce the final triage report as an HTML artifact using `create_html_artifact` and the
template at `@.bob/skills/cve-analyzer/html-report-template.html`. The report MUST include:
...
```json
{
  ...
  "report_path": ".bob/reports/<report_folder>/<issue-number>/triage-<issue-number>-<date>.html",
  "metrics": {
...
In all escalation cases, save a partial HTML report to `.bob/reports/<report_folder>/<issue-number>/`
