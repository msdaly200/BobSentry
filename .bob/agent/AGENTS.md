...
**Exclusion rule:** Do NOT create `context.json` or add entries to `index.json` for:
...
After writing `context.json`, read `.bob/reports/index.json`, append a new entry to the
`sessions` array, and write the file back:
...
{
  "schema_version": "1.0",
  "description": "Index of completed triage sessions. Only includes confirmed or novel-pattern Keycloak GitHub issues stored under .bob/reports/<attack-class-folder>/<issue-number>/. False positives, CVE-named demo folders, and portability demonstrations are excluded.",
