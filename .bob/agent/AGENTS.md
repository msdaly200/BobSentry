...
```bash
cp .bob/reports/setup_realm.py /tmp/keycloak-triage/setup_realm.py
...
```bash
cp .bob/reports/exploit_test.py /tmp/keycloak-triage/exploit_test.py
...
This structure means all artefacts from prior sessions on the same attack class sit in
the same parent folder, making them immediately available to Code mode as reference
material for future sessions on the same pattern.
...
### Docker Log Evidence *(mandatory for all SSRF pattern triages)*
```
[Paste relevant lines from: docker logs keycloak 2>&1 | grep -E "notification endpoint|HttpHostConnectException|Failed to send request|checkUrl"]
```
