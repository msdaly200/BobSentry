...
If the folder contains any subfolders (prior issue numbers), first check each session
subfolder for `context.json`. Prefer the most relevant prior session with `context.json`
when selecting reference material. If no session metadata is present, **read the most
recent `setup_realm.py` and `exploit_test.py`** from those subfolders before writing new
scripts. These are ground-truth working implementations — use them as the primary
reference for:
...
When `context.json` is present, use it to understand the matched pattern, attack type,
affected component, confidence, confirmation signal, verdict, and artifact filenames
before opening prior scripts.
...
**Exclusion rule — `context.json`:** Do NOT create `context.json` for:
- False-positive verdicts (`verdict: "not-vulnerable"`)
- Sessions stored under a CVE-named folder (folder name matches `CVE-*` pattern)

**Exclusion rule — `index.json`:** Do NOT add entries to `index.json` for:
- Sessions stored under a CVE-named folder (folder name matches `CVE-*` pattern)

`not-vulnerable` verdicts **must** be recorded in `index.json` (with `verdict: "not-vulnerable"`) so the system knows the issue was already cleared.
Omit the `context_json` field for `not-vulnerable` entries (no `context.json` is written).

#### Report Format

Produce the final triage report as a Markdown file. The report MUST conform to the
canonical schema defined in:

```
@.bob/references/triage-report-schema.md
```

That document is the **single source of truth** for section order, heading names,
required vs optional sections by session type, and issue-type-specific content
variations. Do not invent section names or ordering — follow the schema exactly.

**Session type** (`plan-only`, `executed`, or `escalated`) determines which sections
are required. Determine the session type before writing the first line.

Key required elements for all session types:
- Front-matter block (issue number, CVE, session type, verdict, severity, date)
- Phase 1 — Guardrails table (all 8 rules)
- Phase 2 — Threat Profile table
- Phase 2 — Extraction & Pattern Matching (extraction table + all-5-pattern match block)
- Phase 3 — Sandbox Configuration table
- Phase 4 — Severity Assessment (CVSS v3.1 score + vector)
- Phase 4 — Bug Triage Checklist (all 6 sections)
...
Immediately after the Markdown report is written (and the exclusion rule above does not
apply), create `context.json` in the same session subfolder:

```
.bob/reports/<report_folder>/<issue-number>/context.json
```

Populate it from the triage session data using this schema:
...
```json
{
  ...
  "project": {
    "name": "keycloak",
    "url": "https://github.com/keycloak/keycloak/issues/<issue-number>"
  },
  "classification": {
    "report_folder": "<report_folder>",
    "matched_cve_pattern": "<matched_cve_pattern or null>",
    "assigned_cve": "<assigned_cve or null>",
  ...
  "outcome": {
    "verdict": "confirmed|not-vulnerable|escalate",
    "confirmed": true|false,
    "severity": "<severity label>",
    "cvss_base_score": 0.0,
    "cvss_vector": "CVSS:3.1/...",
    "triage_date": "<YYYY-MM-DD>",
    "keycloak_version_tested": "<version tested in sandbox, e.g. 26.2.5>"
  },
  "artifacts": {
    "report_markdown": "triage-<issue-number>-<date>.md",
    "setup_script": "setup_realm.py",
    "exploit_script": "exploit_test.py"
  },
  "retrieval_summary": {
    "summary": "<one or two sentence description of the vulnerability and attack path>"
  }
}
```

Required fields: `issue_number`, `project.name`, `classification.report_folder`,
`classification.attack_type`, `signals.keywords`, `signals.confirmation_signal`,
`outcome.verdict`, `outcome.triage_date`, `artifacts.report_markdown`,
`retrieval_summary.summary`. All other fields are strongly recommended but not blocking.

#### Updating index.json

After writing `context.json`, read `.bob/reports/index.json` and **upsert** the entry:

- Search `sessions` for an object whose `issue_number` equals the current issue number.
- **If found:** replace that object in-place with the new entry (do not append a duplicate).
- **If not found:** append the new entry to the end of the `sessions` array.
...
```json
{
  "issue_number": "<issue-number>",
  "report_folder": "<report_folder>",
  "session_path": ".bob/reports/<report_folder>/<issue-number>/",
  "context_json": ".bob/reports/<report_folder>/<issue-number>/context.json",
  "assigned_cve": "<assigned_cve or null>",
  "matched_cve_pattern": "<matched_cve_pattern or null>",
  "attack_type": "<attack_type>",
  "verdict": "confirmed|not-vulnerable|escalate",
  ...
  "novel_pattern": true|false,
  "triage_date": "<YYYY-MM-DD>",
  "keycloak_version_tested": "<version or null>",
  "stale": null
}
```

> **`not-vulnerable` entries:** omit `context_json` (no `context.json` is written).
> All other fields are still required.

> **`keycloak_version_tested`:** copy the version string from `outcome.keycloak_version_tested`
> in `context.json`. Set to `null` if the context file was not written or the field is absent.

> **`stale`:** always write `null` at session-write time. This field is updated manually
> or by a future staleness-check pass when a newer Keycloak release may have resolved the
> vulnerability. When populated it becomes an object:
> `{ "since_version": "27.0", "evaluated_date": "YYYY-MM-DD", "note": "..." }`

Also update the top-level `"generated"` field to today's date.

If `index.json` does not exist, create it with the standard wrapper:

```json
{
  "schema_version": "1.0",
  "description": "Index of completed triage sessions. Includes confirmed, not-vulnerable, and escalated Keycloak GitHub issues stored under .bob/reports/<attack-class-folder>/<issue-number>/. CVE-named demo folders and portability demonstrations are excluded.",
